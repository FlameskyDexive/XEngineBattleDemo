#!/usr/bin/env python3
"""M0 acceptance final: the serve-mode idle loop never pumps XTask so the panel's async
catalog refresh can't complete between tool calls. Inject the real (synchronously built)
catalog snapshot + selection into the live panel via reflection, then capture."""
...
print("[menu]", json.dumps(call_tool("runtime_menu", {"action": "invoke", "path": "Window/Package Management/Package Manager"}, 120))[:160], flush=True)
time.sleep(3)

INJECT = ('var t = System.Type.GetType("XEngine.Editor.GUI.Panels.PackageManagerPanel, XEngine.Editor"); '
          ...
          't.GetMethod("SelectPackage", flags).Invoke(panel, new object[]{ "%s" }); '
          'return "injected:" + snap.Entries.Count + " entries, selected %s";')
...
for pkg, out in [("com.xengine.zonezero", "m0-zonezero-installed.png"),
                 ("com.xengine.unityimporter", "m0-unityimporter-installed.png")]:
    print("[inject]", json.dumps(eval_code(INJECT % (pkg, pkg)))[:240], flush=True)
    time.sleep(1)
    grab(out)
